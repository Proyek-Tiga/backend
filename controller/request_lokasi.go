...
func UpdateRequestStatus(w http.ResponseWriter, r *http.Request) {
	...
	if status.Status == "approved" {
		...
		var request model.RequestLokasi
		err := tx.QueryRow(`SELECT nama_lokasi, kapasitas FROM request_lokasi WHERE request_id = $1`, id).Scan(&request.NamaLokasi, &request.Kapasitas)
		if err != nil {
			tx.Rollback()
		...
		_, err = tx.Exec(
			`INSERT INTO lokasi (lokasi, tiket, created_at, updated_at) VALUES ($1, $2, NOW(), NOW())`,
			request.NamaLokasi, request.Kapasitas,
		)
		if err != nil {
			tx.Rollback()
