...
func AddRequestLokasi(w http.ResponseWriter, r *http.Request) {
	...
	// Insert request lokasi
	result, err := tx.Exec(
		`INSERT INTO request_lokasi (user_id, nama_lokasi, kapasitas, status) VALUES ($1, $2, $3, "pending")`,
	...
	if err != nil {
		tx.Rollback()
		http.Error(w, "Failed to retrieve last inserted ID: "+err.Error(), http.StatusInternalServerError)
		return
	}
